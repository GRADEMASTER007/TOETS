  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden">
        ...
        {/* Chat Message Thread */}
        <div className="p-4 sm:p-5 flex-1 overflow-y-auto space-y-3.5 min-h-[260px] max-h-[400px]">
          {messages.map((m, idx) => (
            <div
              key={idx}
              ...
              {m.matchedCount !== undefined && m.matchedCount > 0 && (
                <div className="mt-1 flex items-center gap-2 text-xs">
                  <span className="text-emerald-700 font-semibold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                    {m.matchedCount} Listings Updated on Screen
                  </span>
                  <button
          ...
          {isSearching && (
            <div className="flex items-center gap-2 text-xs text-slate-500 py-2">
              ...
              <span>Gemini is evaluating listings and ranking by relevance...</span>
            </div>
          )}
        </div>

        {/* Quick Sample Queries */}
        <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 flex items-center gap-1.5 overflow-x-auto scrollbar-none text-[11px]">
          <span className="text-slate-400 shrink-0 font-medium">Try:</span>
          {[
            '3 bedroom house in Sandton under R15k',
            'Emergency plumber in Nairobi',
            'Dubai Marina 2 bed apartment',
            'Solar installer with COC',
          ].map((sample, i) => (
            <button
              key={i}
              onClick={() => {
              ...
              className="px-2 py-0.5 rounded-md bg-white border border-slate-200 text-slate-600 hover:text-slate-900 whitespace-nowrap"
            >
              {sample}
            </button>
          ))}
